...
folder_name = os.path.join('/home/antonipa57/PycharmProjects/Spectra_transfer/spectra_transfer_code/ACVAE/img/org_classifier', folder_name)
...
decoder = Decoder(n_classes_channels=1, in_channels=10, ks_lst=[3,3,3,3], out_channels_lst=[16,16,8,1], stride_lst=[1,1,1,1])
...
for i in range(50):
    ...
    for x, y in class_dataset:
        ...
        y = y.view(-1).unsqueeze(1).repeat(1, 3).view(-1)
        y = y.to(device)
        pred = classifier(x)
...
for epoch in range(epochs):
    ...
    for x_0, x_1, y_0, y_1 in ds:
        ...
        optimizer.zero_grad()
        vae_loss = gaus_neg_log_like + kl_loss + ClsLoss_class + ClsLoss_recon + ClsLoss_convert
        vae_loss.backward()
        optimizer.step()
...
for i in range(5 if x_1.shape[0] > 5 else x_1.shape[0]):
    ...
    xy_plot([wavelen[spectra_0_idx:spectra_last_idx + 1][20:-20] for i in range(len(spectra_))], spectra_,
            labels=['original', 'how transformed should look', 'transformed'],
            xy_labels=['wavelength [nm]', 'intensity [a.u.]'], min_max_xlim=[wavelen[spectra_0_idx+20], wavelen[spectra_last_idx+1-20]],
            title='Random reconstructed spectrum visualization', show=False, save_to=folder_name+f'/no_disc_01_{i}.png')
...
for i in range(5 if x_10.shape[0] > 5 else x_10.shape[0]):
    ...
    xy_plot([wavelen[spectra_0_idx:spectra_last_idx + 1][20:-20] for i in range(len(spectra_))], spectra_,
            labels=['original', 'how transformed should look', 'transformed'],
            xy_labels=['wavelength [nm]', 'intensity [a.u.]'], min_max_xlim=[wavelen[spectra_0_idx + 20],wavelen[spectra_last_idx + 1 - 20]],
            title='Random reconstructed spectrum visualization', show=False, save_to=folder_name+f'/no_disc_10_{i}.png')
...
while frog:
    ...
    with torch.set_grad_enabled(True):
        for epoch in range(round(epochs)):
            ...
            for x_0, x_1, y_0, y_1 in ds:
                ...
                disc_pred = torch.cat([disc_pred_c, disc_pred_r], dim=0)

                loss_d = criterion_d(disc_pred_c, torch.ones_like(disc_pred_c))/2
                loss_d += criterion_d(disc_pred_r, torch.ones_like(disc_pred_r))/2

                loss = kl_loss + gaus_neg_log_like + ClsLoss_class + ClsLoss_recon + ClsLoss_convert + loss_d
    ...
    for i in range(5 if x_01.shape[0] > 5 else x_01.shape[0]):
        ...
        xy_plot([wavelen[spectra_0_idx:spectra_last_idx + 1][20:-20] for i in range(len(spectra_))], spectra_,
                labels=['original', 'how transformed should look', 'transformed'],
                xy_labels=['wavelength [nm]', 'intensity [a.u.]'], min_max_xlim=[wavelen[spectra_0_idx+20], wavelen[spectra_last_idx+1-20]],
                title='Random reconstructed spectrum visualization', show=False, save_to=folder_name+f'/disc_01_{cow}_{i}.png')
    ...
    for i in range(5 if x_10.shape[0] > 5 else x_10.shape[0]):
        ...
        xy_plot([wavelen[spectra_0_idx:spectra_last_idx + 1][20:-20] for i in range(len(spectra_))], spectra_,
                labels=['original', 'how transformed should look', 'transformed'],
                xy_labels=['wavelength [nm]', 'intensity [a.u.]'], min_max_xlim=[wavelen[spectra_0_idx + 20], wavelen[spectra_last_idx + 1 - 20]],
                title='Random reconstructed spectrum visualization', show=False, save_to=folder_name+f'/disc_10_{cow}_{i}.png')